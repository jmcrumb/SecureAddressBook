...
public class Login extends Command {
    ...
    public String execute() throws UserVisibleException, IOException, GeneralSecurityException {
        ...
        if (entry.passwordHash == null) {
            ...
            String hashedPass = Encryption.hashSHA256(password);
            //set up signing keys
            KeyPair kp = generatePublicPrivateKeys();
            PrivateKey priv = kp.getPrivate();
            PublicKey pub = kp.getPublic();
            //encrypt private key
            byte[] b = encrypt(keyToB64(priv), hashedPass);
            String encryptedPriv = bytesToString(b);

            entry = new UserEntry(userid, Encryption.hashBCrypt(newPassword), encryptedPriv, keyToB64(pub));

            database.set(entry);
            if (entry.userId.equals("admin")) {
                entry = new AdminEntry(entry);
                //set up AuditLog
                KeyPair auditKp = generatePublicPrivateKeys();
                AuditLog.setPrivateKey(auditKp.getPrivate(), (String s) -> encrypt(keyToB64(priv), hashedPass));
                AuditLog.setPublicKey(auditKp.getPublic());
            }
            try {
                User.getInstance().setUser(entry, hashedPass);
            } catch (Exception e) {
    ...
    private boolean loginGuard(UserEntry entry) {
        ...
        return (penalizedAttempts <= 0) || (diff > 60 * Math.pow(2, penalizedAttempts));
    }
    ...
    private static final Set<String> topPasswords = new HashSet<>(Arrays.asList(arrayOfPasswords));
